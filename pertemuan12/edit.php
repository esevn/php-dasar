...
if (isset($_POST["submit"])) {
    $data = $_POST;
    if (updateDataStudent($data, $id) > 0) {
...
<body>
    <div class="container flex flex-col items-center justify-center bg-white">
        <form action="" method="POST" class="bg-slate-400 border m-auto py-4 flex w-full flex-col gap-4 border-black items-center justify-center">
            <label for="old_image">Gambar</label>
            <input type="text" name="old_image" id="old_image" required value="<?= $student[0]['image'] ?>">
            ...
            <button type="  " name="submit" class="bg-red-500 px-4 py-4 rounded-md text-white transition-all hover:rotate-180">Kirim data</button>
        </form>
    </div>
</body>
